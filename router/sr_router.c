        ...
        char* interface/* lent */)
{
      ...
      if(flag==1){
        ...
        if(req){
          ...
          while(pkts->next){
            ...
            /*sanity check*/    
            if(cksum(packet + sizeof(struct sr_ethernet_hdr), ntohs(iphdr->ip_len)) != ip_hdr->ip_sum){
              ...
              if(((ip_hdr->ip_ttl)-1) > 0){
                ...
                ip_hdr->ip_ttl--;
                ip_hdr->ip_sum = cksum(packet + sizeof(struct sr_ethernet_hdr), ntohs(iphdr->ip_len));
  ...
  /* ip frame handling*/
    ...
    uint16_t cs = cksum(packet + sizeof(struct sr_ethernet_hdr), ntohs(iphdr->ip_len));
